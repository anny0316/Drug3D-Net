...
from grid.griddataset import grid_Dataset
from grid import gridModel as om
...
class GridTrainer(object):
    ...
    def load_model(self, model, units_conv_coord=64, units_conv_atom=128, units_dense=128, chebyshev_order=3, num_layers=2, loss="mse", pooling="max", isattention=True):
        self.hyper["model"] = model
        self.hyper["units_conv_coord"] = units_conv_coord
        self.hyper["units_conv_atom"] = units_conv_atom
        self.hyper["units_dense"] = units_dense
        self.hyper["chebyshev_order"] = chebyshev_order
        self.hyper["num_layers"] = num_layers
        self.hyper["loss"] = loss
        self.hyper["pooling"] = pooling
        self.hyper["isattention"] = isattention
        self.model = getattr(om, model)(self.hyper)
        self.model.summary()

    def load_grid_data(self, batch=128):
        self.data = grid_Dataset(self.hyper["dataset"], batch=batch)
        ...
        self.hyper['gridshape'] = self.data.gridshape

    def fit_grid(self, model, epoch, batch=128, fold=1, pooling="max", units_conv_coord=64, units_conv_atom=128, units_dense=512, chebyshev_order=3, num_layers=2,
            loss="mse", monitor="val_rmse", mode="min", use_multiprocessing=True, label="", isattention=True, **kwargs):
        ...
        for i in range(1, fold + 1):
            ...
            self.load_grid_data(batch=batch)
            self.data.set_features(**kwargs)
            # self.hyper["num_features"] = self.data.num_features

            # 3. Make model
            self.load_model(model, units_conv_coord=units_conv_coord, units_conv_atom=units_conv_atom, units_dense=units_dense, chebyshev_order=chebyshev_order, num_layers=num_layers, loss=loss,
                            pooling=pooling, isattention=isattention)

            # 4. Callbacks
            log_path = base_path + "{}_c1{}_c2{}_d{}_l{}_p{}_{}{}/".format(batch, units_conv_coord, units_conv_atom, units_dense, num_layers,
                                                                     pooling, label, now.strftime("%m%d%H"))
            tb_path = log_path + "trial_{}/".format(i)

            callbacks = []
            if self.data.task != "regression":
                callbacks.append(Roc(self.data.generator("valid"), self.data.task))
                mode = "max"
            callbacks += [Tensorboard(log_dir=tb_path, write_graph=False, histogram_freq=0, write_images=True),
                          ...
                          EarlyStopping(patience=5, restore_best_weights=True),  # 15, hiv=10
            ...
            self.model.fit_generator(self.data.generator("train"), epochs=epoch,
                                     ...
                                     use_multiprocessing=use_multiprocessing, workers=1)
        ...
        self.log["{}_B{}_C1{}_C2{}_D{}_L{}_P{}".format(model, batch, units_conv_coord, units_conv_atom, units_dense, num_layers, pooling,
                                                 )] = results
